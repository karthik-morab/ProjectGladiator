...
public class oneWayFlightTC extends Base {
	...
	public void NoFromAndNoTo() throws Exception {
		logger=report.createTest("Testing One Way Flight with no From and no Destination");
    	logger.info("Starting the web application");
	...
	logger=report.createTest("Testing One Way Flight with Valid From and no Destination");
    logger.info("Starting the web application");
	driver.manage().window().maximize(); // maximizing window
	...
	logger=report.createTest("Testing One Way Flight if no valid date");
	logger.info("Starting the web application");
